...
if (isset($_POST['id'])) {
    ...
    // Préparer la requête pour restaurer l'étudiant
    $sql = "UPDATE etudiants SET archive = 0 WHERE id = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            // Redirection vers la liste des étudiants archivés après la restauration
            header("Location: etudiant_archiver.php");
            exit();
        } else {
